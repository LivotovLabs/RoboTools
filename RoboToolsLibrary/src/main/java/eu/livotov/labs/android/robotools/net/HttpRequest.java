...
public class HttpRequest implements Closeable {
    ...
    public static HttpRequest get(CharSequence url, Map<? extends CharSequence, ? extends CharSequence> params) throws IOException {
        if(TextUtils.isEmpty(url)) {
            throw new MalformedURLException("Param url must not be empty");
        }
        url = url.toString() + '?' + paramsToUrlencoded(params);
        return get(url);
    }
    ...
    public HttpRequest(CharSequence url, CharSequence requestMethod) throws MalformedURLException {
        this(new URL(url.toString()), requestMethod.toString());
    }
    ...
    public HttpRequest header(CharSequence name, CharSequence value) throws IOException {
        checkConnection();
        getConnection().setRequestProperty(name.toString(), value.toString());
        return this;
    }

    public HttpRequest headers(final Map<? extends CharSequence, ? extends CharSequence> headers) throws IOException {
        for (Map.Entry<? extends CharSequence, ? extends CharSequence> header : headers.entrySet()) {
