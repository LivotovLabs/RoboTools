...
public class CookieStore implements org.apache.http.client.CookieStore {
    ...
    public void saveCookieStore(final Context ctx) throws IOException {
        ...
        editor.putString(CS_KEY, Base64.encodeToString(bos.toByteArray(), Base64.DEFAULT));
        editor.commit();
    }
    ...
    public void readCookieStore(final Context ctx) {
        try {
            SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(ctx);
            ByteArrayInputStream bis = new ByteArrayInputStream(Base64.decode(preferences.getString(CS_KEY, ""), Base64.DEFAULT));
