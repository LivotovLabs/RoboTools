...
//    private RTHTTPClient http = new RTHTTPClient();
    ...
    public void cancelTask(final P task) {
        if (task != null && task.status == Status.Downloading) {
                ...
                try {
                    ...
                    while (retryDownload) {
                        ...
                        if (TextUtils.isEmpty(url)) {
                            Log.e(DownloadService.class.getSimpleName(), String.format("No more mirrors left for task %s, failing download.", downloadTask.getDownloadId()));
                            downloadTask.status = Status.Failed;
                            retryDownload = false;
                        } else {
                            try {
                                ...
                                Log.e(DownloadService.class.getSimpleName(), err.getMessage(), err);
                                retryDownload = downloadTask.supportsMirrors();
                                if (!retryDownload) {
                                    downloadTask.status = Status.Failed;
                                } else {
                                    Log.e(DownloadService.class.getSimpleName(), String.format("Failed download for task %s , url %s , will  now try another mirror.", downloadTask.getDownloadId(), url));
                                }
                            }
                        }
                    }
                    ...
                    switch (downloadTask.status) {
                        ...
                        case Cancelled:
                        case Cancelling:
                            downloadTask.status = Status.Cancelled;
                    ...
                    Log.e(DownloadService.class.getSimpleName(), "Download error: " + downloadTask.getDownloadUrl() + " : " + err.getMessage(), err);
    ...
    private void processSingleDownload(final P job, final String urlString) throws Throwable {
        ...
        currentTask.status = Status.Downloading;

        HttpRequest request = HttpRequest.get(urlString);
        request.connectTimeout(job.getHttpConnectionTimeoutMs());
        request.readTimeout(job.getHttpDataResponseTimeoutMs());
        ...
        currentTask.contentType = request.contentType();
        ...
        InputStream inStream = request.send();

        verifyStream(currentTask, inStream);
        ...
        while (inStream != null && -1 != (count = inStream.read(buffer))) {
            if (currentTask.status != Status.Downloading) {
                break;
            }
        ...
        if (currentTask.status != Status.Downloading) {
            ...
            currentTask.status = Status.Postprocessing;
            updateNotifications();
            currentTask.performDownloadPostprocess();
            currentTask.status = Status.Finished;
        }
        request.close();
        currentTask = null;
    }
    ...
    protected abstract void verifyStream(P task, InputStream is) throws Exception;
    ...
    {
        Idle, Downloading, Postprocessing, Finished, Failed, Cancelling, Cancelled
    }
